...
### 2026-07-30 — FLOW-001 — reject silent recordings before transcription; manual recheck pending

- Agent/session: Codex
- Commit: this commit (`FLOW-001: reject silent recordings`)
- What changed: Added a non-content audio signal measurement during WAV finalization. The controller now treats a finalized recording below the conservative average-amplitude threshold as “No speech detected” and deletes it without contacting Groq or pasting an ASR hallucination. Added regression coverage that an overlapping hold cannot alter the first transaction's transcribe/paste path.
- Verification commands: `pwd`; `scripts/bootstrap.sh`; initial sandbox `scripts/check.sh` (existing localhost mock-server binds denied); approved baseline `scripts/check.sh`; `cargo fmt --check`; `cargo clippy --all-targets -- -D warnings`; `cargo test --locked silent`; `cargo test --locked controller::tests`; final approved `scripts/check.sh`; `git diff --check`.
- Manual acceptance evidence: User manually confirmed the under-300 ms tap, a normal hold/speak/release transaction, and Escape cancellation all pass. User observed that an attempted overlapping hold resulted in no paste, so the full transaction matrix is not accepted. User also observed Groq returning “Thank you” for a held silent recording; this code change rejects low-signal audio before Groq, but needs manual rechecking. User reported `scripts/check.sh` passes. The Codex CLI text input treats injected Ctrl+V as an image-paste command and reports no image on the clipboard; that already-complete PASTE-001 compatibility issue is not changed during this single FLOW-001 session.
- Known limitations or follow-up: Keep `FLOW-001` at `passes: false`. Rebuild/relaunch Echo, then repeat a 300+ ms silent hold and verify no text is pasted and Echo says “No speech detected.” Also repeat a spoken hold/release followed immediately by a second hold while Echo is transcribing, verifying the first transcript pastes once and the second hold is ignored. After FLOW-001 is accepted, investigate Codex CLI/terminal input compatibility as a PASTE-001 bug without changing the documented Ctrl+V behavior until its target semantics are established.
- Next eligible feature: FLOW-001
