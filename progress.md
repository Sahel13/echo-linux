...
### 2026-07-30 — RELEASE-001 — production debug status removed

- Agent/session: Codex
- Commit: this commit (`RELEASE-001: remove production debug status`)
- What changed: Removed the X11 ready labels and the transaction status label from the settings window. Removed the temporary FLOW-001 diagnostic label, per-transaction counters, and `echo.flow` stderr reporting. Dictation recording, transcribing, and timed error feedback remains in the existing non-interactive overlay, and the required non-X11 warning remains visible.
- Verification commands: `pwd`; complete reads of `AGENTS.md`, `docs/PORTING_GUIDE.md`, `progress.md`, and `feature-list.json`; `git log -10 --oneline`; `git status --short --branch`; `scripts/bootstrap.sh`; sandbox baseline `scripts/check.sh` (localhost mock-server binds denied); approved baseline and final `scripts/check.sh`; `cargo fmt`; `cargo test --locked tests::x11_session_is_supported`; `cargo clippy --all-targets -- -D warnings`; `cargo build --locked`; source audit with `rg`; `git diff --check`; approved X11 launch and window capture with `xwininfo`, `xdotool`, and `ffmpeg`.
- Manual acceptance evidence: A capture of the freshly rebuilt window on the active 1920×1200 X11 display showed the settings content ending at the Help and Quit actions, with no ready, transaction-status, or diagnostic text below them. The full suite passed 61 tests with three documented environment-dependent tests ignored.
- Known limitations or follow-up: RELEASE-001 remains failing because this session did not build or run the versioned archive on the Ubuntu 22.04 baseline, inspect and document its dynamic runtime dependencies, verify the archive layout and checksum, or test extraction on a clean matching X11 system without a Rust toolchain.
- Next eligible feature: RELEASE-001

### 2026-07-30 — AUTOSTART-001 — application-managed autostart removed

- Agent/session: Codex
- Commit: this commit (`AUTOSTART-001: remove managed autostart`)
- What changed: Reversed the earlier autostart product decision at the user's direction. Removed the Launch at login switch and status UI, the complete XDG desktop-entry backend, its main-module integration, the persisted `launch_at_login` setting, and the feature specification. Updated the porting guide to leave session startup entirely to users and their desktop environment, window manager, or distribution. Existing version-1 settings containing the removed field remain readable and are rewritten without it.
- Verification commands: `pwd`; complete reads of `AGENTS.md`, `progress.md`, `feature-list.json`, and `docs/PORTING_GUIDE.md`; `git log -10 --oneline`; `git status --short --branch`; `scripts/bootstrap.sh`; sandbox baseline `scripts/check.sh` (localhost mock-server binds denied); approved baseline `scripts/check.sh`; `cargo fmt`; focused settings and UI contract tests; `cargo clippy --all-targets -- -D warnings`; `cargo build --locked`; JSON validation; source/spec audit with `rg`; approved X11 launch with an isolated XDG configuration and a window capture; final approved `scripts/check.sh`; `git diff --check`.
- Manual acceptance evidence: The rebuilt Echo window was launched on the active 1920×1170 X11 display with isolated settings. Visual inspection showed the General section contains lifetime dictated words, Copy last transcript, Help, and Quit, with no launch-at-login switch or status. The temporary process, settings directory, and screenshot were removed after verification.
- Known limitations or follow-up: A desktop entry created by an older Echo build is external user configuration and is intentionally not removed by the application now that all autostart management code is gone. Affected users can remove `~/.config/autostart/io.github.sahel.Echo.desktop` themselves or manage it through their desktop environment. RELEASE-001 remains the next incomplete feature.
- Next eligible feature: RELEASE-001

### 2026-07-30 — PASTE-001 — Vim and Codex terminal paste compatibility fixed

- Agent/session: Codex
- Commit: this commit (`PASTE-001: fix Ghostty terminal paste`)
- What changed: Made paste dispatch target-aware. Ordinary X11 clients retain `Ctrl+V`; recognized terminal emulators use their terminal paste accelerator; and Ghostty uses the focused window's standard GTK `paste` action discovered from its X11-published application ID, unique bus name, and window object path, so neither Vim nor Codex receives a literal `Ctrl+V`. Replaced the narrow hand-rolled clipboard owner with the maintained `clipboard_x11` implementation while retaining the bounded text snapshot and change-aware restoration checks. The Ghostty action path validates the exact application ID and object-path prefix before making the scoped session-bus call.
- Verification commands: Required startup `pwd`, repository/specification reads, `scripts/bootstrap.sh`, sandbox baseline `scripts/check.sh` (localhost mock-server binds denied), approved baseline and final `scripts/check.sh`; `cargo fmt`; focused paste tests; `cargo clippy --all-targets -- -D warnings`; repeated approved ignored live X11 paste tests; isolated Ghostty/Neovim mode and buffer queries; isolated Ghostty/Codex window capture; `git diff --check`.
- Manual acceptance evidence: In Ghostty 1.3.1 on the active X11 session, the final live backend test pasted the fixed text `Echo paste check` into a clean Neovim buffer while `mode(1)` remained `i`; the queried buffer contained exactly that text. A separate fresh Codex 0.146.0 prompt visibly contained `Echo paste check`, with no image-paste error. Both checks exercised the final clipboard owner and focused Ghostty GTK action, and all isolated windows, sockets, files, and screenshots were removed afterward.
- Known limitations or follow-up: Ghostty was the live terminal environment available for the reported Vim and Codex failures. Unit tests cover classification and `Ctrl+Shift+V` selection for other common terminal window classes, but those terminal emulators were not installed for live verification. RELEASE-001 remains failing and unchanged.
- Next eligible feature: RELEASE-001

### 2026-07-31 — RELEASE-001 — reproducible release archive prepared; Ubuntu baseline verification pending

- Agent/session: Codex
- Commit: this commit (`RELEASE-001: prepare release archive`)
- What changed: Added a reproducible versioned x86-64 release builder, archive/checksum verifier, Ubuntu 22.04 multi-stage Docker recipe, release README, and gitignore rules for generated release output. The archive contains the executable, README.txt, license, and the 256×256 Echo icon at `share/icons/hicolor/256x256/apps/echo.png`. Replaced the generic Linux microphone header image with the bundled Echo icon. The packaged mark and README wordmark are byte-for-byte copies of the macOS source assets; the PNG combines the app icon's original dark gradient and mark.
- Verification commands: `pwd`; complete reads of `AGENTS.md`, `docs/PORTING_GUIDE.md`, `progress.md`, and `feature-list.json`; `git log -10 --oneline`; `git status --short`; `scripts/bootstrap.sh`; sandbox baseline `scripts/check.sh` (localhost mock-server binds denied); approved baseline and final `scripts/check.sh`; `cargo fmt`; focused UI-contract test; `packaging/build-release.sh`; `packaging/verify-release.sh dist/echo-linux-x86_64-0.1.0.tar.gz`; `tar -tzf`; `sha256sum -c`; `ldd`; source-asset SHA-256 comparison; `git diff --check`.
- Manual acceptance evidence: On this x86-64 Arch host, the generated archive checksum verified, its listed layout contained exactly the required binary, README.txt, LICENSE, and icon, and every dynamic library resolved on the host. This is not claimed as Ubuntu 22.04 or clean-X11 evidence. The host is Arch with glibc 2.44, so its binary must not be published as the Ubuntu 22.04 release.
- Known limitations or follow-up: `RELEASE-001` remains `passes: false`. This environment has neither Docker nor Podman, so the required clean Ubuntu 22.04 build and no-Rust-toolchain extraction run could not be exercised. On a host with Docker BuildKit, run `docker build --target smoke-test -f packaging/Dockerfile.ubuntu-22.04 .`, then `docker build --target archive --output type=local,dest=dist -f packaging/Dockerfile.ubuntu-22.04 .`, followed by `packaging/verify-release.sh dist/echo-linux-x86_64-0.1.0.tar.gz`. Then extract the exported archive on a clean Ubuntu 22.04 X11 desktop without Rust, run Echo, and visually confirm the settings window opens. Record its `ldd` output and the resulting checksum before setting `passes` to true.
- Next eligible feature: RELEASE-001
